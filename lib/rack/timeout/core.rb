...
module Rack
  class Timeout
    ...
    attr_reader \
      ...
      :paths_to_skip,     # Paths to skip
      :term_on_timeout

    def initialize(app, service_timeout:nil, wait_timeout:nil, wait_overtime:nil, service_past_wait:"not_specified", term_on_timeout: nil, paths_to_skip: nil)
      @paths_to_skip     = (paths_to_skip || ENV.fetch("RACK_TIMEOUT_PATHS_TO_SKIP", "").split(",").reject(&:empty?)).map{ |regexp_str| Regexp.new regexp_str }
